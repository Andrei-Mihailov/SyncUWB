...
%     Compare_signal = reshape(Compare_signal, 1, size(Compare_signal, 1)*size(Compare_signal, 2));
%     Compare_signal = [Compare_signal zeros(1,N)];%+1к нулей
        ...
        if (flag == 0)&&(Flag_count <= preamble_length/20)
            if count < 3
                ...
                if (strt > size(Compare_signal, 2) - data_N-10 - N)&&(strt < size(Compare_signal, 2) - data_N - N)
                    ...
                    s(1,size(Compare_signal, 2)+1 - data_N - N:end) = Receive_Data(Compare_signal,start_data,Q);%запись данных
                    break;
           ...
           % принимаем импульсы с периодом
                     ...
                     if x == strt + Q% если импульс следует за прошлым с данным периодом Q
                        s(x) = Compare_signal(x); % аписываем импульс
                     else if x == strt + Q + 1% записываем следующий импульс
                             s(x) = Compare_signal(x);
                             Flag_count = Flag_count + 1;
                             flag = 0;
                             % записываем в массив указатели на отсчеты
                             % преамбулы
